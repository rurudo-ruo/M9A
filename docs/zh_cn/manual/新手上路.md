...
M9A 基于 [MaaFramework](https://github.com/MaaXYZ/MaaFramework) ，您至少需要一台能够在模拟器上流畅运行 1999 的设备来运行 M9A 。
...
<summary>不知道设备是什么架构？点击查看Mac用户查看处理器架构方法</summary>
...
模拟器支持详情可参阅 MAA 文档。由于 MAA 与 M9A 在框架上不尽相同，故**仅供参考**。

<details>

<summary>展开 MAA 模拟器支持文档</summary>
...
M9A 支持主流模拟器，但您需要设置模拟器模拟器分辨率以达到运行要求。请自行在模拟器设置中更改分辨率。
...
M9A 支持通过命令行（ MaaPiCli.exe ）或图形化界面（ MFAAvalonia.exe ）来执行任务。但在使用之前，您必须正确解压 M9A 的压缩包。

> [!CAUTION]
> 不要在压缩软件直接打开程序！

对于一般用户，推荐通过 **MFAAvalonia.exe** 使用 M9A 。
...
> [!CAUTION]
> 首次启动 MFA 时， M9A 将进行初始化。直到 `日志` 板块显示 **`AgentServer 启动`** **`任务已全部完成`** 前，请不要关闭 MFA 窗口或点击 `停止任务` 按钮，否则可能会导致 M9A 无法正常工作。
...
击主界面左下角的齿轮按钮进入 M9A 设置界面。在设置界面，您至少应当配置 **`更新设置`** ，同时推荐有多开和自启需求的用户配置 `启动设置` 。

<details open>
  <summary><b>更新设置</b></summary>
  <blockquote>
    <ul>
      <li>
        <b>资源下载源</b>：指定 MFA 内更新 UI 或资源时使用的下载源。
        <ul>
      ...
      <li>
        <b>CDK</b>：Mirror 酱下载源的授权码。
        <ul>
          <li>有 Mirror 酱 CDK 的用户请填写，无则留空。</li>
          <li>若资源下载源为 Mirror 酱但未填写 CDK，则无法更新。</li>
        </ul>
      </li>
    </ul>
  </blockquote>
...
<details>
  <summary><b>启动设置</b></summary>
  <blockquote>
    <ul>
      <li>
        <b>软件路径</b>：指定模拟器可执行文件路径。
        <ul>
          <li>5.0 以下 MuMu 12：<code>{安装目录}\shell\MuMuPlayer.exe</code></li>
          <li>5.0 及以上 MuMu 12：<code>{安装目录}\nx_device\12.0\shell\MuMuNxDevice.exe</code></li>
          <li>示例：<code>D:\MuMuPlayer-12.0\shell\MuMuPlayer.exe</code></li>
        </ul>
      </li>
      <li>
        <b>附加命令</b>：指定模拟器启动参数。
        <ul>
          <li>最少填写 <code>-v X</code>，X 为多开号（无多开为 0）。</li>
          <li>多开号可在 MuMu 多开器中查看。</li>
        </ul>
      </li>
    </ul>
    <details>
      <summary>自动获取模拟器路径方法</summary>
      <blockquote>
        <ol>
          <li>以 MuMu 模拟器为例，确保桌面有 1999 的模拟器快捷方式。</li>
...
您至少应当配置 **`资源类型`** 和 **`连接`** 。配置错误时将无法使用 M9A 。
  ...
  <details open>
      <summary><b>资源类型</b></summary>
      <blockquote>
        <ul>
          <li>必须选择与模拟器中安装的 1999 区服类型一致。</li>
          <li>截至 M9A v3.8.1，支持：<b>官服</b>、<b>B服</b>、<b>OPPO服</b>、<b>国际服（EN）</b>、<b>国际服（JP）</b>、<b>小米服</b>、<b>QQ服</b>。</li>
        </ul>
      </blockquote>
    </details>
    <details open>
      <summary><b>连接</b></summary>
      <blockquote>
        <ul>
          <li>
            <details>
              ...
              <blockquote>
                <ul>
                  ...
                  <li>请选择正确的模拟器和 ADB 地址组合。</li>
                  <li>启动 M9A 后会自动检测所有已启动的模拟器。</li>
                  <li>如仅有一个模拟器，M9A 会自动连接。</li>
                </ul>
              </blockquote>
            </details>
          </li>
          <li>
            <details>
              ...
              <blockquote>
                <ul>
                  <li>右上角有 <b>自定义</b>、<b>刷新</b>、<b>连接状态</b> 三个图标。</li>
                  <li><b>自定义</b>：手动编辑 ADB 参数（一般无需修改）。</li>
                  <li><b>刷新</b>：重新检测所有已启动模拟器。</li>
                  <li><b>连接状态</b>：绿色为已连接。</li>
                  <li>更多说明详见 <a href="./连接设置.md">连接设置</a>。</li>
                  <li><img src="https://github.com/user-attachments/assets/8e3f0f90-4e0d-46e2-b718-837a6e9ae152" alt="image_440"></li>
                </ul>
              </blockquote>
            </details>
          </li>
    ...
    <details>
      <summary><b>任务列表</b></summary>
      <blockquote>
        <ul>
          <li>点击任务名前复选框启用/禁用任务。</li>
          <li>点击任务名后齿轮按钮查看 <b>任务设置</b> 与 <b>任务说明</b>。</li>
          <li>拖动任务名可调整任务顺序。</li>
          <li>点击 <b>开始任务</b>，M9A 按顺序执行已启用任务。</li>
        </ul>
      </blockquote>
  </details>
