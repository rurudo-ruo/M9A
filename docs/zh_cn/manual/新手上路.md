...
**`更新设置`**
  ...
  `资源下载源` 用于指定您在 MFA 内更新 UI 或资源时使用的下载源。M9A 会尝试从 Mirror 酱或 GitHub 服务器下载更新资源。
  
  - 默认情况下，M9A 会使用 `Mirror 酱` 作为下载源。
  
  - 没有购买 Mirror 酱的用户请修改此项为 `GitHub` 。
  
  `CDK` 用于指定用户在选择 `Mirror 酱` 作为下载源时的 Mirror 酱 CDK。
  
  - 有 Mirror 酱 CDK 的用户请在此处填入自己的 CDK，没有购买 Mirror 酱的用户无需填写。

  - 若您在`资源下载源`处选项为 `Mirror 酱`，但没有在此处填写 CDK，则 M9A 无法进行更新。

**`启动设置`**

  `软件路径` 用于指定模拟器的可执行文件的路径， `附加命令` 用于指定打开模拟器可执行文件时附加的命令行参数。配置错误时 M9A 将无法启动正确的多开。 

- `软件路径`

  以 5.0 版本以下的 MuMu 模拟器 12 为例，`软件路径`通常为 `{安装目录}\shell\MuMuPlayer.exe` 。请自行找到模拟器可执行文件的路径。

  以您将 MuMu 模拟器安装在D盘为例，`{安装目录}` 为 `D:\MuMuPlayer-12.0`，则您需要在`软件路径`中输入 `D:\MuMuPlayer-12.0\shell\MuMuPlayer.exe` 。

  如果您更新了 5.0 版本的 MuMu 模拟器 12 ，您需要重新设置 `软件路径` 。
...
- `附加命令`
    ...
    以 5.0 版本以下的 MuMu 模拟器 12 为例，`附加命令`最少您应当填写 `-v X` 。v 为小写字母，X 为数字，具体数字取决于 1999 所在的 MuMu 模拟器的多开号。例如，如果您没有使用多开功能，`附加命令` 您应当填写 `-v 0` 。
